...
contract WebAuthnAccount is BaseAccount, TokenCallbackHandler, UUPSUpgradeable, Initializable, AccessController {
    ...
    //custom
    function _validateWebAuthnWithLoginServiceSignature(bytes memory signature, bytes32 userOpHash, bool dryRun)
        internal
        returns (uint256 validationData)
    {
        (
    ...
    //custom
    function _validateWebAuthnSignature(bytes memory signature, bytes32 userOpHash, bool dryRun)
        internal
        returns (uint256 validationData)
    {
        (
